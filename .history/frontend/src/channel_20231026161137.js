...
// 监听点击botton查看频道消息
// 监听'channel-list'上的点击事件
document.querySelector('.channel-list').addEventListener('click', function(event) {
  // 检查是否点击了'info'按钮
  if (event.target.tagName === 'BUTTON' && event.target.innerText === 'Info') {
    // 获取channel-information元素
    const channelInfo = document.getElementById('channel-information');
    // 更改其display属性为'block'
...
function generateChannelList(channelList) {
  ...
  const createChannelElement = (channel, joinBtn = false) => {
    ...
    // 在li标签中添加button，点击时可以查看频道消息
    const infoButton = document.createElement("button");
    infoButton.innerText = "Info";
    channelElementDom.appendChild(infoButton);
...
export function getChannelList() {
  http.get("/channel").then((res) => {
    ...
    localStorage.setItem("channelsInformation", res.channels);
    const channelsInformation = localStorage.getItem("channelsInformation");
      console.log("channelsInformation", channelsInformatio);
