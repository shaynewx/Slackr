...
function showChannelMessage(channelId, start = 0) {
  ...
  .then((res) => {
    ...
    if (item.reactions) {
      // 添加反应
      const reactionsDom = document.createElement("div");
      reactionsDom.className = "message-reactions";
          ...
          if (this.classList.contains('delete')) {
              ...
              reactionMenu.querySelectorAll('.emoji').forEach(emoji => {
                  emoji.addEventListener('click', function() {
                      ...
                      // TODO: 根据需要保存或显示反应
                      http.post(`/message/react/${currentChannelId}/${currentMessageIdmessageId}`, 
                      {reaction}, true)
                      .then((res) => {
                        console.log(res);
                        showChannelMessage(currentChannelId);
