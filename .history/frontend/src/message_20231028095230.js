...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const closeEditMessage = document.getElementById("edit-new-message-submit");
      ...
      option.addEventListener('click', function(e) {
          ...
          if (this.classList.contains('delete')) {
              ...
              editMessageSubmit.addEventListener("click", () => {
                const messageContent = editMessageInput.value;
                //input没有被编辑时，发送按钮不可用
                if (!messageContent.trim() || messageContent === editMessageInput.defaultValue) {
                  console.log("message is null");
                  showErrorMessage("Message can not be empty");
                  return;
                }
                ...
                currentChannelId = channelId;
                console.log("currentChannelId", currentChannelId);
                http.put(`/message/${channelId}/${currentMessageId}`, {
                  message: messageContent
                }, true)
                .then((res) => {
                  console.log(res);
              ...
              // 提交时，发送请求，并且关闭modal，渲染新消息，新消息指明已经被编辑，以及时间戳
