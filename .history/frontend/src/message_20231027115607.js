...
function showChannelMessage(channelId, start = 0) {
  ...
  http.get(`/message/${channelId}?start=${start}`).then((res) => {
    ...
    messages.forEach(item => {
      ...
      // fetch发送人的信息
      console.log("message sender id", item.sender)
      if (item.sender !== userId) {
        user.fetchUserInfo(item.sender);
        .then((senderUserInfo) => {
          console.log("senderUserInfo", senderUserInfo);
        }

      }
      // fetchSenderInfo(item.sender).then((senderUserInfo) => {
      //   console.log("sen derUserInfo", senderUserInfo);
      // });
