    ...
    messages.forEach((item) => {
      ...
      if (item.reacts && item.reacts.length > 0) {
        ...
        item.reacts.forEach((reactObj) => {
          ...
          reactionDom.textContent = reactObj.react + " " + (reactObj.count || 1); 
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // unreact 功能
  function unreactToMessage(messageId, reaction) {
    http.post(`/message/unreact/${currentChannelId}/${messageId}`, { react: reaction }, true)
      .then((res) => {
        console.log(res);
  ...
  document.querySelector(".context-option.react").addEventListener("click", function (e) {
  ...
  reactionMenu.querySelectorAll(".emoji").forEach((emoji) => {
    ...
    // TODO: 判断用户是否已经为此消息添加了此反应
      const userAlreadyReacted = false;  // 根据你的数据结构来获取这个值

    if (userAlreadyReacted) {
      // 如果用户已经添加了该反应，那么取消该反应
      unreactToMessage(currentMessageId, reaction);
    } else {
      // 否则，添加该反应
      http.post(`/message/react/${currentChannelId}/${currentMessageId}`, { react: reaction }, true)
        .then((res) => {
          console.log(res);
          showChannelMessage(currentChannelId);
        });
