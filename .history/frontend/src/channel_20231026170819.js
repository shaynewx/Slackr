...
function createChannelHearder(channel, static = false) {
...
function generateChannelList(channelList) {
  ...
  unjoinedChannels.forEach((channel) => {
    ...
    createChannelHearder(channel, false);
  });

  //TODO:获取频道列表后再获取频道信息
  // 生成channel information 
  // 写一个监听事件，当点击li标签时，找到localstorage中的channel id，然后发送get请求，获取频道信息
  // 生成频道信息，即在id为channel-information-header的div中添加频道信息
  // 首先生成一个div存放channel name，再生成一个div存放五个button，包括频道详情、人员列表、搜索、邀请、退出频道
  // 
