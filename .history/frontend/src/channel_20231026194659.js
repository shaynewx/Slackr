...
function generateChannelList(channelList) {
 ...
 // 展示频道详情信息
  function showChannelDetails(storedChannelInfo,stat) {
    console.log("showChannelDetails is called with channel:", storedChannelInfo);
    ...
    namePara.textContent = `Name: ${storedChannelInfo.name}`;
    channelDetails.appendChild(namePara);
    // Description
    const descriptionPara = document.createElement('p');
    descriptionPara.textContent = `Description: ${storedChannelInfo.description}`;
    ...
    privatePara.textContent = storedChannelInfo ? "Type: Private Channel" : "Type: Public Channel";
    channelDetails.appendChild(privatePara);

    // Create Time
    const createTimePara = document.createElement('p');
    createTimePara.textContent = `Create Time: ${storedChannelInfo.createdAt}`;
    channelDetails.appendChild(createTimePara);

    // Creator
    const creatorPara = document.createElement('p');
    creatorPara.textContent = `Creator: ${storedChannelInfo.creator}`;
    ...
    if (stat===true) {
        ...
        editButton.addEventListener('click', function() {
          // TODO: 这里是编辑频道信息的逻辑
            ...
            // 使用PUT请求更新频道信息
            http.put(`/channel/${storedChannelInfo.id}`, {
                name: storedChannelInfo.name, 
                description: storedChannelInfo.description
            }).then(res => {
                // TODO:更新成功后的处理(不确定是否正确)
  ...
  function createChannelHearder(channel, stat = false) {
    ...
    channelElementDom.addEventListener("click", () => {
      http.get(`/channel/${channelId}`).then((channelInfo) => {
        ...
        localStorage.setItem("channelInfo"));
      });

      const storedChannelInfo = localStorage.getItem("channelInfo");

      console.log("storedChannelInfo", storedChannelInfo);
      ...
      buttonsInfo.forEach(btnInfo => {
          ...
          if (btnInfo.action) {
              button.addEventListener('click', function() {
                  btnInfo.action(storedChannelInfo, stat);
              });
          }
